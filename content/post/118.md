...
`document.querySelector('#span1').childNodes[0]` (テキストノード)に対してイベントリスナーを設定しているので、 `document.querySelector('#span1').textContent = this.value` とすることで、イベントリスナーを設定したテキストノード自体がどこかへ消えてしまっているように見えました。

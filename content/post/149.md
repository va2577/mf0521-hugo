...
引用のページの最初には、 Anaconda をインストールする記載がありました。
...
```python
df['DATE'] = pd.Series([datetime.fromtimestamp(time.mktime(time.localtime(x / 1000))) for x in df[0]])
df['USDJPY'] = pd.Series([float(x) for x in df[4]])
```

プロットしたときに、 Unix エポックのミリ秒だとラベルが日付の書式で表示されなかったので、 datetime にしました。

それから、レートが "111.222" のような string 型になっていて、プロットしたときに、 `TypeError: Empty 'DataFrame': no numeric data to plot` のようなエラーになりました。
なので、 float 型にしました。

DataFrame の中の型の変換とか、もっと簡単にできるのかな。
...
* http://nbviewer.jupyter.org/gist/va2577/3e6a3f475b65d064267339830e158c85
...
久しぶりに Python や Pandas を使って、忘れてしまっていました。
Pandas の Series や DataFrame は始めて使ったとき、便利だと思ったのですが、書き残しておかなかったので、また調べなおしながら、書き残してもおきたいと思います。

Python の Jupyter Notebook と pandas は簡単にデータを分析できて便利なのでけっこう好みです。
すぐにグラフにすることもできました。
Chart.js みたいなグラフを作るライブラリーはないのかな。

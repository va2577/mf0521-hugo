...
Google Treands で検索してみました。

<script type="text/javascript" src="https://ssl.gstatic.com/trends_nrtr/1328_RC04/embed_loader.js"></script>
<script type="text/javascript">
trends.embed.renderExploreWidget("TIMESERIES", {"comparisonItem":[{"keyword":"itext 5","geo":"","time":"today 12-m"},{"keyword":"itext 7","geo":"","time":"today 12-m"},{"keyword":"itextsharp","geo":"","time":"today 12-m"}],"category":0,"property":""}, {"exploreQuery":"q=itext%205,itext%207,itextsharp&date=today 12-m,today 12-m,today 12-m","guestPath":"https://trends.google.co.jp:443/trends/embed/"});
</script>
...
`ColumnText.ShowTextAligned` は、この 1 行だけで出力が完了するようですが、 `PdfContentByte.ShowTextAligned` は `PdfContentByte.BeginText();` と `PdfContentByte.EndText();` を記述しなければいけないようなので、少し手間がかかります。

`PdfContentByte.ShowTextAligned` は Align を基準に用紙の外側までどこまでも文字列を出力するようです。
折り返しはなく、 1 行を出力するときに使いそうです。
    ...
    public void Method1()
    {
        ...
        using (var document = new iTextSharp.text.Document(reader.GetPageSizeWithRotation(1)))
        {
            ...
            var contentByte = writer.DirectContent;
            {
                ...
                // エラー CS0012  型 'Matrix' は、参照されていないアセンブリに定義されています。アセンブリ 'System.Drawing, Version=2.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a' に参照を追加する必要があります。	ConsoleApp1 C:\Users\user\Source\Repos\ConsoleApp1\ConsoleApp1\Class4.cs  42  アクティブ
...
文字列の出力も座標を把握しなければいけないのですが、コードに直接座標を記述して、トライ＆エラーしながら微調整しました。
素早く座標を把握する方法はあるのかな？
